        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.businesses.update", [$business->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control ckeditor {{ $errors->has('summary') ? 'is-invalid' : '' }}" name="summary" id="summary">{!! old('summary', $business->summary) !!}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('business_categories') ? 'is-invalid' : '' }}" name="business_categories[]" id="business_categories" multiple required>
                        @foreach($business_categories as $id => $business_category)
                            <option value="{{ $id }}" {{ (in_array($id, old('business_categories', [])) || $business->business_categories->contains($id)) ? 'selected' : '' }}>{{ $business_category }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('location') ? 'is-invalid' : '' }}" type="text" name="location" id="location" value="{{ old('location', $business->location) }}">
