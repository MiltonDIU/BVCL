...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.service.assign-to") }}" enctype="multipart/form-data">
                @csrf
...
{{--                            <option value="{{ $id }}" {{ old('service_status_id') == $id ? 'selected' : '' }}>{{ $service_status }}</option>--}}
{{--                        @endforeach--}}
{{--                        --}}
    ...
    @endforeach
                ...
                <div class="form-group">
                    <label class="required" for="assigns">{{ trans('cruds.service.fields.assign') }}</label>
                    <div style="padding-bottom: 4px">
                        <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                        <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                    </div>

                    <select class="form-control select2 {{ $errors->has('assigns') ? 'is-invalid' : '' }}" name="assigns[]" id="assigns" multiple required>
                        @foreach($assigns as $id => $assign)
                            <option value="{{ $id }}" {{ in_array($id, old('assigns', [])) ? 'selected' : '' }}>{{ $assign }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('assigns'))
                        <div class="invalid-feedback">
                            {{ $errors->first('assigns') }}
                        </div>
                    @endif
                    <span class="help-block">{{ trans('cruds.service.fields.assign_helper') }}</span>
                </div>
                <div class="form-group">
