...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.trainingApply.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.training-applies.update", [$trainingApply->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('training') ? 'is-invalid' : '' }}" name="training_id" id="training_id">
                        @foreach($trainings as $id => $training)
                            <option value="{{ $id }}" {{ (old('training_id') ? old('training_id') : $trainingApply->training->id ?? '') == $id ? 'selected' : '' }}>{{ $training }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                        @foreach($users as $id => $user)
                            <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $trainingApply->user->id ?? '') == $id ? 'selected' : '' }}>{{ $user }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('user'))
                        <div class="invalid-feedback">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('is_status') ? 'is-invalid' : '' }}" name="is_status" id="is_status">
                        ...
                        @foreach(App\Models\TrainingApply::IS_STATUS_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('is_status', $trainingApply->is_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
